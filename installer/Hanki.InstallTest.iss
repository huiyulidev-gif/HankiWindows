...
#define InstallTestRoot "..\dist-dev\0.2.0-install-test"
#define InstallTestPayload "..\dist-dev\0.2.0-install-test\Hanki-0.2.0-dev-install-test-win-x64"
...
OutputDir={#InstallTestRoot}
OutputBaseFilename=HankiSetup-0.2.0-dev-install-test
...
VersionInfoDescription={#MyAppName} Windows 개발 테스트 설치 프로그램
...
[Files]
Source: "{#InstallTestPayload}\*"; DestDir: "{app}"; Flags: ignoreversion recursesubdirs createallsubdirs
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        '한키 로컬 단축키와 설정 데이터도 삭제할까요?' + #13#10 +
        '아니오를 선택하면 나중에 다시 설치할 때 사용할 수 있습니다.',
        mbConfirmation,
        MB_YESNO) = IDYES then
      begin
